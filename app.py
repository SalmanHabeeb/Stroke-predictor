...
def predict(gender, married, age, residence, glucose_level, height, weight, work_status, disease, smoker):
    ...
    age = (df['Mean'][0] - age)/df['Standard_deviation'][0]
    residence = int(residence == 'Urban')
    glucose_level = (df['Mean'][1] - glucose_level)/df['Standard_deviation'][1]
    bmi = weight/((height/100)**2)
    bmi = (df'Mean'][2] - bmi)/df'Standard deviation'][2]
...
age = st.number_input('Age :', min_value = 0.3, max_value = 80, value = 25)
residence = st.selectbox('Residence type :', ['Urban', 'Rural'])
glucose_level = st.number_input('Average Glucose Level(in mg/dL) :', min_value = 10, max_value = 400, value = 120)
height = st.number_input('Height(in cm) :', min_value = 30, max_value = 220, value = 160)
weight = st.number_input('Weight(in kg) :', min_value = 5, max_value = 120, value = 70)
